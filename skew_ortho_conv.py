...
def l2_normalize(tensor, eps=1e-12):
    norm = torch.sqrt(torch.sum(tensor.float() * tensor.float()))
...
# Class for testing from original repo
class OriginalSOC(nn.Module):
    def __init__(self, tensor, in_channels, out_channels, kernel_size=3, stride=1, padding=None, 
                 bias=True, train_terms=5, eval_terms=12, init_iters=50, update_iters=1, 
                 update_freq=200, correction=0.7, device="cuda"):
        ...
        self.random_conv_filter = nn.Parameter(tensor.to(device),
                                               requires_grad=True)
        ...
        with torch.no_grad():
            u1, v1, u2, v2, u3, v3, u4, v4 = fantastic_four(conv_filter, 
                                                num_iters=self.init_iters, device=self.device)
        ...
        self.correction = nn.Parameter(torch.Tensor([correction]).to(self.device), requires_grad=False)
            
        self.enable_bias = bias
        if self.enable_bias:
            self.bias = nn.Parameter(
                torch.Tensor(self.out_channels).to(self.device), requires_grad=True)
        else:
            self.bias = None
